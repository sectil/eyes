...
describe('oyun seçenekleri', () => {
  it('TrueDepth yoksa her zaman dokunma; varsa kayıtlı seçim touch değilse göz', () => {
    const st = memStorage()
    expect(loadSnakeOpts(false, st)).toEqual({ control: 'touch', walls: 'classic', practiced: false })
    expect(loadSnakeOpts(true, st)).toEqual({ control: 'eyes', walls: 'classic', practiced: false })
    expect(saveSnakeOpts({ control: 'touch', walls: 'wrap' }, st)).toBe(true)
    expect(JSON.parse(st.getItem(OPTS_KEY))).toEqual({ control: 'touch', walls: 'wrap', practiced: false })
    expect(loadSnakeOpts(true, st)).toEqual({ control: 'touch', walls: 'wrap', practiced: false })
  ...
  it('bozuk kayıt ve erişim hatası varsayılana düşer', () => {
    ...
    expect(loadSnakeOpts(true, boom)).toEqual({ control: 'eyes', walls: 'classic', practiced: false })
    expect(saveSnakeOpts({ control: 'eyes', walls: 'wrap' }, boom)).toBe(false)
    expect(saveSnakeOpts({ control: 'eyes', walls: 'wrap' }, null)).toBe(false)
    expect(loadSnakeOpts(true, null)).toEqual({ control: 'eyes', walls: 'classic', practiced: false })
  })
})
