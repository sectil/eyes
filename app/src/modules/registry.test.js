...
describe('modül soketi: gerçek modüller', () => {
  it('hepsi geçerli, sorun yok', () => {
    ...
    expect(registry.modules.map((m) => m.id).sort()).toEqual(['awareness', 'blink', 'breath', 'breath-count', 'daily', 'dalga', 'fark-ettin', 'notice', 'quick-look', 'reading', 'routine', 'snake', 'tek-bakis', 'track', 'weekly'])
  })
  ...
  it('kayıtları tanır; silinecek anahtarlar modüllerden gelir', () => {
    ...
    expect(registry.inSection('practice').map((m) => m.id)).toEqual(['quick-look', 'fark-ettin', 'track', 'tek-bakis', 'snake', 'breath', 'dalga', 'notice'])
  })
})
